...
{
    public class StockReceive : NancyModule
    {
        const string DoctypeName = "StockReceive";
        const string DatabaseTable = "vStockReceive";
        const string DetailTable = "vStockReceiveDetail";
        ...
        public static void Test()
        {
            ...
            Utils.print(stockReceive.GetAll());
        ...
        private void Run()
        {
            ...
            Post($"/{DoctypeName}/add", args =>
            {
                ...
                List<StockReceiveDetailObject> detailObjectsList = new List<StockReceiveDetailObject>();

                foreach (dynamic detailObject in detailList)
                {
                    detailObjectsList.Add(new StockReceiveDetailObject(
        ...
        private string Add(string docNo, string dateString, string description, string refDocNo, List<StockReceiveDetailObject> detailObjectsList)
        {
            if (Auth.Login(userSession))
            {
                AutoCount.Stock.StockReceive.StockReceiveCommand cmd =
                AutoCount.Stock.StockReceive.StockReceiveCommand.Create(userSession, userSession.DBSetting);
                AutoCount.Stock.StockReceive.StockReceive doc = cmd.AddNew();
                ...
                foreach (StockReceiveDetailObject detailObject in detailObjectsList)
                {
                    AutoCount.Stock.StockReceive.StockReceiveDetail detail = doc.AddDetail();
        ...
        private string Edit(string docNo, string dateString, string description, string refDocNo, List<StockReceiveDetailObject> detailObjectsList)
        {
            if (Auth.Login(userSession))
            {
                AutoCount.Stock.StockReceive.StockReceiveCommand cmd =
                AutoCount.Stock.StockReceive.StockReceiveCommand.Create(userSession, userSession.DBSetting);
                AutoCount.Stock.StockReceive.StockReceive doc = cmd.Edit(docNo);
    ...
    internal class StockReceiveDetailObject
    {
        ...
        public StockReceiveDetailObject(string itemCode, string uom, decimal quantity, decimal unitCost = 0M)
        {
