...
{
    public class StockIssue : NancyModule
    {
        const string DoctypeName = "StockIssue";
        const string DatabaseTable = "vStockIssue";
        const string DetailTable = "vStockIssueDetail";
        ...
        public static void Test()
        {
            ...
            Utils.print(stockIssue.GetSingleDetail("ISS-001"));
        ...
        private void Run()
        {
            ...
            Post($"/{DoctypeName}/add", args =>
            {
                ...
                List<StockIssueDetailObject> detailObjectsList = new List<StockIssueDetailObject>();

                foreach (dynamic detailObject in detailList)
                {
                    detailObjectsList.Add(new StockIssueDetailObject(
        ...
        private string Add(string docNo, string dateString, string description, string refDocNo, List<StockIssueDetailObject> detailObjectsList)
        {
            if (Auth.Login(userSession))
            {
                AutoCount.Stock.StockIssue.StockIssueCommand cmd =
                AutoCount.Stock.StockIssue.StockIssueCommand.Create(userSession, userSession.DBSetting);
                AutoCount.Stock.StockIssue.StockIssue doc = cmd.AddNew();
                ...
                foreach (StockIssueDetailObject detailObject in detailObjectsList)
                {
                    AutoCount.Stock.StockIssue.StockIssueDetail detail = doc.AddDetail();
        ...
        private string Edit(string docNo, string dateString, string description, string refDocNo, List<StockIssueDetailObject> detailObjectsList)
        {
            if (Auth.Login(userSession))
            {
                AutoCount.Stock.StockIssue.StockIssueCommand cmd =
                AutoCount.Stock.StockIssue.StockIssueCommand.Create(userSession, userSession.DBSetting);
                AutoCount.Stock.StockIssue.StockIssue doc = cmd.Edit(docNo);
    ...
    internal class StockIssueDetailObject
    {
        ...
        public StockIssueDetailObject(string itemCode, string uom, decimal quantity, decimal unitCost = 0M)
        {
